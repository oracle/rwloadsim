...
void rwlexpreval ( rwl_estack *stk , rwl_location *loc , rwl_xeqenv *xev , rwl_value *ret)
{
  ...
  for (explen=0; j==0; explen++)
  {
    ...
    if (bit(xev->t1flags,RWL_THR_DEVAL))
    {
      ...
      for (j=0; j<explen; j++)
      {
	//fprintf(stderr, " %d ", stk[j].evaltype);
		...
		{ /* open the file */
		  ...
		  else
		  {
			...
			filnam = rwlenvexp(xev, loc, cnp->sval);
		      ...
		      case RWL_T_PIPETO:
		      case RWL_T_PIPEFROM:
			rwlexecerror(xev, loc, RWL_ERROR_RAW_FILE_ONLY_RWA, vv->vname);
		      break;

		      default:
			rwlexecsevere(xev, loc, "[rwlexpreval-badrawfilasn:%s;%d]"
			  , vv->vname, stk[i].filasn);
		    }
		  }
