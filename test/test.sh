...
then
  ...
  # note that 323..326 tests were used by removed CQN
...
if test "${NLS_DATE_FORMAT+x}" = x
then
  nls_date_format_was_set=yes
  nls_date_format_save="$NLS_DATE_FORMAT"
else
  nls_date_format_was_set=no
  nls_date_format_save=''
fi

for i in $testlist
do
  if test $nls_date_format_was_set = yes
  then
    export NLS_DATE_FORMAT="$nls_date_format_save"
  else
  ...
  case $i in
    ...
    ;;
    20|21|22|69|41|68|72|74|76|81|88|368)
  ...
  thisisgood=yes
  case $i in
    28|29|35|39|40|43|60|126|143|144|145|136|151|152|155|156|166|172|197|207|208|214|229|230|235|312|313|325|353|354|381|389|398|403|407|445|448|450) diffok=yes;;
  esac
  case $i in
    21|41|68|88|270) ociok=yes;;
  esac
  case $i in
    151|156|184|177|158|197|320|373) errok=yes;;
  esac
  ...
  case $i in
    ...
    367)
      rm -f testres/$i.out testres/$i.err
      for arg in '' '--oo=42' '--no-ooswitch' '--ooval=22' '--oo 42 --ooswitch --oovalb 42' '--oovala=42 --oo=42' '--ooval 333 --oovalb=444 --ooswitch'
      do
	test $echo = yes && echo '>>>>>>>>' $i: $rwloadsim $arg $i.rwl
	$rwloadsim -q $i.rwl $arg >> testres/$i.out 2>> testres/$i.err
	runstat=$?
      done
      ;;
    282)
      rm -f testres/$i.out testres/$i.err
      for arg in '' '--ab-c 42' '--no-yes-orno' '--ab-c "-42"' '--no-oryes' '--hel-lo Howdy_verden --ab-c 424242' '--hel-lo Howdy_verden --no-oryes --ab-c 42424'
      do
	test $echo = yes && echo '>>>>>>>>' $i: $rwloadsim $i.rwl $arg
	$rwloadsim -q $arg $i.rwl >> testres/$i.out 2>> testres/$i.err
	runstat=$?
      done
      ;;
    ...
    305)
      rm -f testres/$i.out testres/$i.err
      test $echo = yes && echo '>>>>>>>>' $i: $rwloadsim $i.rwl
      $rwloadsim -q $i.rwl >> testres/$i.out 2>> testres/$i.err
      runstat=$?
      test $echo = yes && echo '>>>>>>>>' $i: env RWLOADSIM_PATH=demo:public $rwloadsim $i.rwl
      env RWLOADSIM_PATH=demo:public $rwloadsim -q $i.rwl >> testres/$i.out 2>> testres/$i.err
      runstat=$?
    ;;
    144|145|146|147)
    ...
    300)
      ...
      ;;
    379)
      test $echo = yes && echo '>>>>>>>>' $i: $rwloadsim $extraarg $pre $i.rwl 379-2.rwl $post --generate=379 ';' ./379 ';' rm -f 379
      $rwloadsim -q $extraarg $pre $i.rwl 379-2.rwl $post --generate=379 > testres/$i.out 2> testres/$i.err
      runstat=$?
      ./379 -q >> testres/$i.out 2>> testres/$i.err
      rm -f 379
      ;;
    355)
      test $echo = yes && echo '>>>>>>>>' $i: env RWLTESTLOG355=/tmp/355.log $rwloadsim $extraarg $pre $i.rwl $post
      env RWLTESTLOG355=/tmp/355.log $rwloadsim -q $extraarg $pre $i.rwl $post > testres/$i.out 2> testres/$i.err
      runstat=$?
      ;;
    *)
  ...
  if test $i = 158 && test $runstat -eq 139
  then
    echo segmentation fault in test $i, rerun required
    segvcount=`expr $segvcount + 1`
    thisisgood=no
    segv158=yes
  fi
  if test $segv158 = no && diff $diffarg testres/$i.out.good testres/$i.out
  then
  ...
  if test $segv158 = no && diff $diffarg testres/$i.err.good testres/$i.err 
  then
...
if test '(' $badcount -gt 0 ')' -o '(' $segvcount -gt 0 ')'
then
...
if test '(' $ociokcount -gt 0 ')' -o '(' $diffokcount -gt 0 ')' -o '(' $badcount -gt 0 ')' -o '(' $segvcount -gt 0 ')'
then
