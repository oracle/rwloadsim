...
Release 3.3 is now the latest.
For all details, see [CHANGELOG.md](CHANGELOG.md).

Note that there have been several updates to the repository, the lastest one was in
release 3.2.2 found in the rwl322.sql script.
Note that all repository update scripts must be run in sequence.
See [docs/INSTALL.md](docs/INSTALL.md) for details.

## Branches

The branches that should be used by ordinary users are named after the release, e.g. 3.3.0.
The master branch is no longer used, it has been replaced by a branch called oramain.
...
At present, branch 3.3.0 is the release branch.
